...
transact <- sqlQuery(channel, "select * from tr")
close(channel)

## Q6
customerstats <- subset(transact, select = -c(msno,
                                              registration_init_time,
                                              methother,planother,pricother,sub_churn))
...
library(car)
set.seed(100)
df_all = rbind(transformed,transformed2)
df_train = transformed
df_test = transformed2

labels = df_train['is_churn']
df_train = df_train[-grep('is_churn', colnames(df_train))]
trains = data.matrix(df_all[,-1])
nrow(trains)
nrow(labels)
train <- trains[c(1:264260),]
labels$is_churn <- as.numeric(as.character(labels$is_churn))
X_test <- subset(df_test, select = -c(is_churn))
                                              
xgb <- xgboost(data = train, 
               label = labels$is_churn, 
               eta = 0.1,
               max_depth = 15, 
               nround=25, 
               subsample = 0.5,
...
y_pred <- predict(xgbl, data.matrix(X_test[,-1]))
predict_last <- head(y_pred,907471)
dfp = data.frame(te_clmsno$msno, predict_last) 
